...
esp_err_t ui_tasks_init(void)
{
    ...
    lv_label_set_text(s_title_label, "Background Tasks");

    /* Scrollable task list container */
    s_task_list = lv_obj_create(s_tasks_scr);
    lv_obj_set_size(s_task_list, 340, 260);
    lv_obj_align(s_task_list, LV_ALIGN_CENTER, 0, 10);
    ...
    /* Stop click events from propagating to parent back_cb */
    ...
    lv_obj_set_width(s_back_hint, 280);
    lv_obj_align(s_back_hint, LV_ALIGN_BOTTOM_MID, 0, -36);
    lv_label_set_text(s_back_hint, "Tap to go back");

    /* Detail overlay (hidden by default) */
    s_detail_panel = lv_obj_create(s_tasks_scr);
    lv_obj_set_size(s_detail_panel, 320, 280);
...
void ui_tasks_refresh(void)
{
    ...
    if (info->task_count == 0) {
        ...
        for (int i = 0; i < info->task_count; i++) {
            ...
            lv_obj_align(name_lbl, LV_ALIGN_LEFT_MID, 18, -8);
            ...
            lv_obj_align(sub_lbl, LV_ALIGN_LEFT_MID, 18, 10);
            ...
            lv_obj_align(status_icon, LV_ALIGN_RIGHT_MID, -4, 0);
        }
    }
